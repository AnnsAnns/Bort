...
According to a presentation by China Telecom, they have a 44% wired IPv6 adoption and a 80% mobile user adoption and is aiming for 100% by 2030.

There also lunatics that would rather change the IPv4 protocol to allow for more addresses than to switch to IPv6, but that whole thing is so absurd and bonkers that I won't even go into it.[^5]
...
Nowadays Hamachi[^4] is extremely business-oriented and not really usable for private persons anymore, but the same issues are resurfacing, just in a different form.
...
So what can you do? Well, it's time for my disclosure that this isn't meant to glorify Cloudflare, but they do have a really nice solution for this problem, and it's free.

Cloudflare offers a service called "Tunnels" and "Access" that allows you to tunnel your traffic through their network **without** exposing your IP address or even having a public IPv4 address. 
...
You can even use "Cloudflare Access" to restrict access to your tunnel to only certain people, which is quite nice if you want to host something private. 

For example, you can expose Portainer to the internet without having to worry that much. 
That probably made Network Admins shiver in fear but rest assured, Cloudflare allows you to greatly secure the entire tunnel (OAuth2 + Passkey + Security Key). 
