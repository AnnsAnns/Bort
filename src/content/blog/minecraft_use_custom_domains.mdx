...
:::note
The content of this blog post is still correct, however, Playit seems to have, unintentionally, made this experience feel a bit buggy.
In an effort to combat the extremely common practice of port scanning for Minecraft servers without a whitelist, the SRV
redirects started breaking within Minecraft, still allowing users to connect, but not properly showing the status of the server.
:::
