...
In the end the boiled down to a few options, till I finally settled on the [IONOS 1€/month VPS](https://www.ionos.de/server/vps).
...
Technically I could also use tailscale exit nodes to route traffic from my home server through the IONOS VPS, however, I don't want
tailscale to be a hard dependency in my network. Right now tailscale is more like a convenient little helper tool that allows me to easily access my servers
and using it for exit nodes would make it a hard dependency that I couldn't simply remove if I wanted to.
